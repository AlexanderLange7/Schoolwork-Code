...
public:
  ...
  /**
   * Does this tree contain value k.
   * 
   * \return True if does contain, False otherwise
   */
  bool contains(const T& k) const { return recContains(pRoot, k); }
  
  /**
   * What is the height of the tree.
   * 
   * \return the longest branch of the tree
   */
  unsigned height() const { return recHeight(pRoot); }
...
private:
   ...
   * 
   */ 
  bool recContains(Node *pCurr, const T& k) const;
  
  unsigned recHeight(Node *pCurr) const;
 ...
 * 
 */ 
template <class T> bool Tree<T>::recContains(Node *pCurr, const T& k) const {
  using namespace std;
  ...
  if (pCurr != 0) {
    ...
    while(i != pCurr->children.end() && flag == 0) {
       flag = recContains(*i, k);
 ...
 * 
 */
template <class T> unsigned Tree<T>::recHeight(Node* pCurr) const {
  using namespace std;
