...
public:
  ...
  public:
     ...
     */
    Node() : tile(), CurCost(), children(), pParent(0) {}
     ...
     * node is the root.
     */
    Node(const T &t, Node *pPar) : tile(t), CurCost(), children(), pParent(pPar) {}
...
public:
  ...
  public:
     ...
     */
    void addChild(const T &t, const double &cost);
   ...
   */
  Tree(const T &t) : pRoot(new Node(t, 0)), n(1u) {}
  ...
  /**
   * Name: root
   * Purpose: return the root of the tree, as a pointer to the node
   */
  Node* root() const { return pRoot; }

  /**
   * Name: contains
   * Purpose: Check the tree to see if there exist the data passed in parameter
   * Return: returns the output of containsNode, private function
   */
  bool containsNode(const T &t, Node* pCurr) const;
  ...
  void findShortest(const T &finish, Node* shortest);


private:

  void findShortest(Node* pCurr, const T &finish, Node* shortest);
...
template <class T> void Tree<T>::findShortest(const T &finish, Node* shortest) {
  findShortest(pRoot, finish, shortest);
}

template <class T> void Tree<T>::findShortest(Node* pCurr, const T &finish, Node* shortest) {
  if(pCurr->children.empty()) {
    if(pCurr->tile == finish && pCurr->CurCost < shortest->CurCost) {
      shortest = pCurr;
    }
  } else {
    for(Node* i : pCurr->children) {
      findShortest(i, finish, shortest);
    }
  }
}
 ...
 */
template <class T> void Tree<T>::Handle::addChild(const T &t, const double &cost) {
  Node* pAdd = new Node(t, pCurr);
  pAdd->CurCost = pCurr->CurCost + cost;
  pCurr->children.push_back(pAdd);
  pTree->n++;
}
...
template <class T> Tree<T>::Tree(const Tree<T> &tree) {
  ...
  pRoot = new Tree<T>::Node(tree.pRoot->tile, 0);
...
typename Tree<T>::Node *Tree<T>::copy(Node *pCurr, Node *pPar) {
  ...
  Node *pN = new Node(pCurr->tile, pPar);
 ...
 */
template <class T> bool Tree<T>::containsNode(const T &t, Node *pCurr) const {
  if(pRoot->tile == t) {
    return true;
  } else {
      for(Node* i : pCurr->pParent->children) {
        if(i->tile == t) {
          return true;
        }
      }
...
template <class T> bool Tree<T>::containsNode(const T &t, Node *pCurr) {
  ...
  if ( pCurr != 0 ) {
    ...
    while (i != pCurr->children.end()) {
      if ( i -> tile == t) {
        ...
        return containsNode(t, *i);
      }
      i++;
    } // while pCurr has children
    
  } // if pCurr is not empty
  
  return false;
  
} // end of containtsNode
*/