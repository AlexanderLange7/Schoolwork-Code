...
public:
  ...
  /**
   * IsProper Public function
   * Returns boolean for whether or not the tree is proper 
   * Calls the private helper function properHelper
   * Authors: Dan and Alex
   * Date: 10/30/2017
   */
  bool isProper() const { return properHelper(pRoot); }
...
private:
  ...
  /**
   * IsProper Private function
   * Returns boolean for whether or not the tree is proper
   * Takes the parameter of pRoot for the tree
   * Authors: Dan and Alex
   * Date: 10/30/2017
   */
  bool properHelper(Node *pCurr) const;
...
typename SimpleBST<T>::Node *SimpleBST<T>::remove(const T &val, Node *pCurr) {
   ...
   */
  template <class T>
  bool SimpleBST<T>::properHelper(Node *pCurr) const {
  
  //While we're not at the bottom of the tree
    //while(pCurr->pLeft !=0 && pCurr->pRight !=0){
      while (pCurr !=0){
      //Iterate through the tree.
      //If at any point only one of the nodes in a child isn't null, return false.
      if((pCurr->pLeft == 0 && pCurr->pRight !=0) || (pCurr->pRight == 0 && pCurr->pLeft!=0)){
        return false;
      } // end if
      
      //Call the private properHelper function for both children
      return properHelper(pCurr->pLeft);
      return properHelper(pCurr->pRight);
    
    } // end of while
    //Once we've gotten through the whole tree, we return true, and our BST is proper.
