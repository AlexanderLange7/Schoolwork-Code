...
public:
  ...
  /**
   * THIS IS THE NEW CONSTRUCTOR BY YOURS TRULY
   * 
   * ALEX AND JOHN
   * 
   * 
   ...
   * 
   */
   //Turn an array given to us into a heap
  IntMinHeap(int* pA, unsigned n){
  //Loop through the Array
  //Push an infinite value onto zero, So our heap is actually a heap. >_> I forgot this and was freaking out foreverrrr.
  ...
  //Go through the Heap
   for(int i = n/2; i >= 1; i--){
    //MinHeapify and make it a minheap
    minHeapify(i);
    }
  }
 
  //DECLATATION OF TOSORTEDARRAY IS HERE. FULL FUNCTION FOUND AT BOTTOM.
...
/**
 * THIS IS OUR TOSORTEDARRAY METHOD, It turns a heap into a sorted array.
 * 
 * ALSO BY ALEX AND JOHN 
 * 
 * 
 * 
 * 11/10/2017
 ...
 */
 void IntMinHeap::toSortedArray(int* pA, unsigned n){
   ...
   for(unsigned int i = 0; i<n; i++){
     //Remove the smallese value from the Heap and place in the array.
    pA[i] = removeMin();    
